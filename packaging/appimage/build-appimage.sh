...
# Generate AppImage using linuxdeploy. linuxdeploy bundles the curses
# library the binary links against (libncursesw); the terminfo database is
# expected to be present on the host (it is, on any normal Linux desktop).
linuxdeploy \
  --appdir="$APPDIR" \
...
# ("owner/repo"); fall back to the defaults for local runs.
