...
area_header *opxpack::getNextArea()
{
    ...
    area_header *tmp = new area_header(ID + 1,
        ...
        (x ? "OPX personal" : "OPX"),
        areas[ID].attr | (cMsgNum ? ACTIVE : 0), cMsgNum, 0, 35, 71);
    ID++;
    return tmp;
}
    ...
    for (int c = 0; c < maxareas; c++) {
        ...
        if (!al->isCollection()) {
            ...
            for (int d = 0; d < ll->noOfLetter(); d++) {
                ...
                long offset = body[c - 1][d].pointer - MSG_HEAD_SIZE;
                putlong(frec.offset, offset);

                frec.flags = ((stat & MS_READ) ? FDX_READ : 0) |
                             ((stat & MS_REPLIED) ? FDX_REPLIED : 0);
                frec.marks = (stat & MS_MARKED) ? FDX_TAGGED : 0;
                frec.msgtype = ((stat & MS_PERSTO) ? 'D' : ' ');
        ...
        if (destfile) {
            while ((c = fgetc(orgfile)) != EOF) {
                if (c == '\001') {
                    c = fgetc(orgfile);
                    int x;
                    bool isReply = (c == 'R'), isPoint = (c == 'T'),
                         isInet = (c == 'I');

                    if (isReply || isPoint || isInet) {
                        for (x = 0; x < (isInet ? 8 : (isPoint ? 4 : 6)); x++)
                            fgetc(orgfile);
                    }
                    x = 0;
                    while ((c != EOF) && (c != '\n')) {
...
bool opxreply::makeOffConfig()
{
    ...
    for (int areaNo = 0; areaNo < maxareas; areaNo++) {
        ...
        if (!(attrib & COLLECTION)) {
            ...
            offrec.scanned = ((((attrib & ACTIVE) && !(attrib & DROPPED))
                             || (attrib & ADDED)));
