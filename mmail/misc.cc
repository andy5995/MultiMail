...
const char *fromAddr(const char *source)
{
    ...
    bool bracket = (*index == '<');
    const char *end = bracket ? strchr(index, '>') : index - (*index == '(');
...
void headdec(const char *source, const char *cset, char *dest)
{
    ...
    if (source)
        for (isqp = isb64 = false, b64buf = b64count = 0; *source; source++) {
            c = *source;
