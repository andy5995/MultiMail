...
void pktbase::initBody(ndx_fake *tmpndx, int personal)
{
    ...
    for (x = 0; x < numMsgs; x++) {
        ...
        body[current][areas[current].nummsgs].pointer = tmpndx->pointer;
        body[current][areas[current].nummsgs++].msgLength = tmpndx->length;
...
void pktbase::fidocheck(letter_header &mhead)
{
    ...
    if (na.isSet)
        if (!na.point) {
            s = strstr(bodyString->getText(), "\001FMPT");
            if (s)
                sscanf(s, "\001FMPT%u\n", &na.point);
        }
    ...
    for (int c = 0; c < d; c++)
        if (x[c][0]) {
            if (!hello && (!strncasecmp("hello", x[c], 5) ||
                ...
                if (!goodbye && !strncasecmp("goodbye", x[c], 7))
