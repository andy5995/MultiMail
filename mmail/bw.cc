...
area_header *bluewave::getNextArea()
{
    ...
    bool inet = isPers ? false : (areas[ID].network_type == INF_NET_INTERNET);
    ...
    area_header *tmp = new area_header(ID + 1, isPers ? "PERS" :
        ...
        (char *) areas[ID].echotag)), (isPers ? "Blue Wave personal" :
        "Blue Wave"), flags_cooked, totmsgs, numpers, from_to_len,
    ...
    if (areas[AreaID].network_type == INF_NET_INTERNET) {
        ...
        if (!mhead.getMsgID()) {
            ...
            if (s) {
                ...
                if (s2) {
                    final = new char[strlen(s) + strlen(s2) + 2];
...
void bluewave::initInf()
{
    ...
    for (int d = 1; d < maxConf; d++) {
        fseek(infFile, (infoHeaderLen + (d - 1) * infoAreainfoLen), SEEK_SET);
...
void bluewave::initMixID()
{
    ...
    if (hasPers) {
        ...
        for (c = 1; c < maxConf; c++)
            if ((mixID[c] != -1) && getshort(mixRecord[mixID[c]].numpers)) {
        ...
        if (!al->isCollection()) {
            ...
            for (int d = 0; d < ll->noOfLetter(); d++) {
                ...
                stat = ((xtiRec.flags & XTI_HAS_READ) ? MS_READ : 0) |
                       ...
                       ((xtiRec.flags & XTI_IS_PERSONAL) ? MS_PERSTO : 0) |
                       ((xtiRec.flags & XTI_HAS_SAVED) ? MS_SAVED : 0) |
...
letter_header *bwreply::getNextLetter()
{
    ...
    const char *msgid;
    net_address na;
    bool isInet = (current->uplRec.network_type == INF_NET_INTERNET);
    if (isInet) {
        na.isInternet = true;
    ...
    if (orgfile) {
        ...
        if (destfile) {
            if (l->uplRec.network_type == INF_NET_INTERNET) {
                ...
                if (l->msgid)
                    fprintf(destfile, "\001References: %s\r\n", l->msgid);
                if (l->newsgrps)
                    fprintf(destfile, "\001Newsgroups: %s\r\n", l->newsgrps);
                if (l->extsubj)
                    fprintf(destfile, "\001Subject: %s\r\n", l->extsubj);
            }
