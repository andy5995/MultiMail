...
void PacketListWindow::MakeActiveCore()
{
    ...
    bool inhome = hlen && (hlen <= len) && !strncmp(home, currDir->name, hlen);
...
void PacketListWindow::oneLine(int i)
{
    ...
    if (absPos < noDirs) {
        absPos = sprintf(tmp, "  <%.28s", packetList->getName());
        ...
        *tmp2++ = '>';
        ...
        sprintf(&tmp[20], "%12lu", (unsigned long) packetList->getSize());
    }
...
searchret PacketListWindow::oneSearch(int x, const char *item, int mode)
{
    ...
    if ((retval == False) && (x >= noDirs) && (mode < s_pktlist)) {
...
void PacketListWindow::renamePacket()
{
    if (active >= noDirs) {
        ...
        if (ui.savePrompt(question, answer) && answer[0] &&
            ...
            bool changeit = !(packetList->exists(expanswer));

            if (changeit) {
                Select();
                changeit = !(packetList->changeName(expanswer));
