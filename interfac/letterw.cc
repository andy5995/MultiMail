...
void LetterWindow::lineCount()
{
    ...
    sprintf(tmp, "%9d/%-10d%3d%%    ", position + 1, NumOfLines, percent);
...
void LetterWindow::UpdateHeader()
{
    ...
    int i = sprintf(tmp, "%.*s", maxSubjWidth, mm.letterList->getSubject());
    letterconv_in(tmp);
    header->put(3, 8, tmp);
    header->clreol(3, i + 8);
...
bool LetterWindow::Save(int stype)
{
    ...
    if (keepname[stype][0])
        ...
        case 2:  // Marked
        case 1:  // All
            sprintf(filename, ntemplate[stype], mm.areaList->getName());
            break;
        case 0:  // This one
            sprintf(filename, ntemplate[0], mm.areaList->getName(),
        ...
        if (fd) {
            ...
            case 1:
                for (int i = 0; i < num; i++) {
                    ...
                    if ((stype == 1) ||
                        (mm.letterList->getStatus() & MS_MARKED))
...
void LetterWindow::write_header_to_file(FILE *fd)
{
    ...
    const char *head[items] = {
        mm.packet->getBBSName(), mm.areaList->getDescription(),
...
searchret LetterWindow::search(const char *item)
{
    ...
    for (int x = position + 1; (x < NumOfLines) && (found == False); x++) {
