                             ...
                             int items)
{
    ...
    while (!result) {
        for (p = selectors, curitem = 0; curitem < items; curitem++, p++) {
            z = strlen(*p);
        ...
        for (p = selectors, curitem = 0; (curitem < items) && !result;
             curitem++, p++)
...
int Interface::savePrompt(const char *prompt1, char *filename)
{
    ...
    return question.getstring(2, 3, filename, 54, C_LLSAVETEXT, C_LLSAVEGET);
}
...
{
    ShadowedWin rwin(3, strlen(message) + 4, (LINES >> 1) - 2, C_WTEXT);
...
bool Interface::back()
{
    ...
    case arealist:
        ...
        if (!abortNow && goodbye) {
            ansiFile(goodbye, goodbye->getName(), mm.packet->isLatin());
            goodbye = 0;
        }
        mm.Delete();
    ...
    if (searchItem) {
        ...
        if (stateok) {
            ...
            default:
                ...
                result = currList->search(searchItem, searchmode);
                if ((result != True) && restorepos)
            ...
            if (result == False)
                if (state == searchstate) {
                    ...
                    if (state == letterlist)
                        ...
                        if (state == arealist)
                            mm.Delete();
                    changestate((statetype)((int) state - 1));
                    searchNext();
                }
            else
                if (result == Abort) {
    ...
    while (!(end || abortNow)) {
        ...
        if (((state == letter_help) || (state == ansi_help))
            ...
            case MM_F2:
                ...
                case letterlist:
                case letter:
                    if (!mm.checkForReplies() || WarningWindow(
