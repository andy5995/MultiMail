...
unsigned char StringFile::nextchar()
{
    ...
    if (fromFile) {
        ...
        if (!result && msgBody) {
            ...
            if (msgBody) {
                srcStr = (const unsigned char *) msgBody->getText();
                curpos = srcStr;
                result = *curpos++;
            }
        }
...
void AnsiWindow::update(unsigned char c)
{
    ...
    static const chtype acstrans[] = {
        ...
        ACS_LRCORNER, ACS_URCORNER, ACS_LLCORNER, ACS_BTEE, ACS_TTEE,
        ACS_LTEE, ACS_HLINE, ACS_PLUS, ACS_LTEE, ACS_LTEE, ACS_LLCORNER,
        ACS_ULCORNER, ACS_BTEE, ACS_TTEE, ACS_LTEE, ACS_HLINE, ACS_PLUS,
        ACS_BTEE, ACS_BTEE, ACS_TTEE, ACS_TTEE, ACS_LLCORNER,
        ACS_LLCORNER, ACS_ULCORNER, ACS_ULCORNER, ACS_PLUS, ACS_PLUS,
...
void AnsiWindow::MakeChain()
{
    ...
    do {
        c = source.nextchar();
#ifdef LIMIT_MEM
        if (maxfreemem() < ((long) NumOfLines * sizeof(AnsiLine *) + 0x200)) {
            c = 0;
            blen = 2;
        }
#endif
...
void AnsiWindow::statupdate(const char *intro)
{
    ...
    sprintf(tmp, " %.*s |%s: %-*.*s %s", pnlen, pn, (intro ? intro : mainstat),
...
void AnsiWindow::lineCount()
{
    ...
    sprintf(tmp, "Lines: %6d/%-6d %3d%%", position + 1, NumOfLines, percent);
...
void AnsiWindow::Delete()
{
    ...
    if (oldcolorx + oldcolory)
        init_pair((oldcolorx << 3) + oldcolory, oldcolorx, oldcolory);
    ...
    case MM_MOUSE:
        if (0 == mm_mouse_event.y)
            ...
            if ((LINES - 1) == mm_mouse_event.y)
                ...
                if ((mm_mouse_event.y > (LINES >> 1)) || (0 == position))
                    KeyHandle(' ');
                else
                    KeyHandle(KEY_PPAGE);
        break;
